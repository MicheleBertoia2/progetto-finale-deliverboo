...
@section('content')
    <div class="container ">
        <div class="py-5 d-flex align-items-center">
            ...
            <a href="{{ route('admin.dishes.create') }}" class="btn mybadge mx-3 p-2">Nuovo Piatto <i
        ...
        @foreach ($dishes as $dish)
            <div class="card m-1 p-0 overflow-hidden" style="width: 286px;">
                ...
                @if(str_contains($dish->image_path, 'http://') || str_contains($dish->image_path, 'https://'))
                    ...
                    <img style="object-fit: cover; width: 100%; height: 100%;" src="{{  $dish?->image_path }}" alt="{{ $dish->name }}">
                @elseif(str_contains($dish->image_path, "resources/img/placeholder-img.png" ))
                    {{-- <img src="{{ Vite::asset($dish->image_path) }}" alt="{{ $dish->name }}"> --}}
                    <img style="object-fit: cover; width: 100%; height: 100%;" src="{{ Vite::asset($dish->image_path) }}" alt="{{ $dish->name }}">
                @else
                    {{-- <img src="{{ asset('storage/' . $dish?->image_path) }}" alt="{{ $dish->name }}"> --}}
                    <img style="object-fit: cover; width: 100%; height: 100%;" src="{{ asset('storage/' . $dish?->image_path) }}" alt="{{ $dish->name }}">
                @endif
